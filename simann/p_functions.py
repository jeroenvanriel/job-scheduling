...
def localSearch(difference, temperature, good_accept, bad_accept, null_move=False):
    """Only accepts good-moves."""
    if difference == 0: # null-move
        return null_move
    elif difference < 0: # good-move
        return True
    else: # bad-move
        return False

def linearBadmoveAccept(difference, temperature, good_accept, bad_accept, null_move=False):
...
def exponentialDecay(difference, temperature, good_accept, bad_accept, null_move=False):
...
def exponentialDecayGreedy(difference, temperature, good_accept, bad_accept, null_move=False):
    """Use an exponential function of the difference."""
    if difference == 0: # null-move
        return null_move
    elif difference < 0: # good-move
